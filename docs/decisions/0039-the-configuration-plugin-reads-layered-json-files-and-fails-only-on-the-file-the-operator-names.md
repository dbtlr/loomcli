...
- **A wrong value is a usage failure.** A file that parses but holds a value the option cannot take fails the run with code 2 from any file, named by its label, as a Boolean variable outside the grammar does.
