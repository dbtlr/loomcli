...
- 2026-09-24: Proposed [ADR-0031](0031-a-plugin-supplies-facts-to-another-plugins-projection-through-a-collecting-extension.md) supersedes the sentence that a subpath imports nothing from a sibling. A subpath may import a sibling's declarations module at `<subpath>/extension` to supply values to a collecting extension the sibling declares, and never a sibling's entry, middleware, or views module. An application that installs one plugin still bundles one plugin. It binds when ADR-0031 is accepted.
