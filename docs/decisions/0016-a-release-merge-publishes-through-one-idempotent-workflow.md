...
4. *Record* runs under the `release` environment with `contents: write`. It creates the annotated tag `v<version>` at the commit named in the version's registry provenance when the tag is absent. It creates the GitHub Release on that tag when absent, with the changelog section as notes, and attaches the tarball fetched back from the registry, so the record holds the bytes consumers receive. Reads after writes retry with backoff, because the registry propagates with a delay.
...
**Recovery has three modes, in order.** A transient failure is a re-run of the failed job. A defect in the workflow is fixed on `main`, then the workflow is dispatched on `main`. The plan job rejects any run whose ref is not `main`, so a dispatch always runs at `main`'s head, and it publishes only while the participating tree is unchanged since the cut. Anything else is fixed forward, and what that means depends on whether bytes reached the registry. A version that never published, for example one blocked by a tag at the wrong commit or by a changed participating tree, is abandoned as unpublished and the next cut supersedes it. A version whose published bytes are defective stays on the registry with its tag and Release, and the next cut supersedes it. Nothing is ever repaired in place.

**Trust is bound by identity, not by a secret.** The npm trusted publisher for `@loomcli/core` names this repository, the `release.yml` filename, and the `release` environment, and permits direct `npm publish`. Both writing jobs run under the environment, and its deployment branch policy, restricted to `main`, is the only control that pins publication, the tag, and the Release to `main`, because the publisher does not bind a branch and a dispatch can name any ref. The plan job's ref check fails such a run early and visibly; the environment policy is what makes the write impossible. The environment holds no secrets and has no reviewers. Tags are not protected; the only actor who could create one outside the workflow is the maintainer.
...
The material-change baseline stops depending on a tag. An abandoned unpublished version has no tag, so the compiler and guard derive the baseline from the first-parent commit that set the current version, which is consistent with ADR-0012's rule that tags never determine versions. The release-cut prerequisite accepts a previous version that was explicitly abandoned as unpublished, and the next cut's narrative says so. A published version with defective bytes needs no exception: it is tagged, on the registry, and simply followed by the next version.
