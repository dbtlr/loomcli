...
- 2026-09-15: [ADR-0028](0028-plugins-run-code-at-lifecycle-hooks-and-middleware-reads-the-request.md), proposed, supersedes the seam's placement: local parsing and validation run ahead of the chain and their fault is held and raised at the dispatch boundary, the point the chain reaches when its last middleware continues, so a middleware reads the request through `request`, a wrapper ahead of help still reaches help's takeover, and a takeover keeps the fault suppression this record promised, help on an invalid invocation included, while a validator now runs ahead of it. The one chain, declared activation, deferred loading, the outcome `next()` resolves, and unwinding order all stand. The consequence above that a provider-style plugin opens a resource before validation is superseded with the placement: it now opens it after validation, on an invocation validation passed or whose fault is held. The rejected option "schemas and presence rules on plugin options" keeps its ruling for options in the globals table; an option a plugin declares on one Command through `onCommandAttach` is a local option and carries a schema like any other.
- 2026-09-24: [ADR-0032](0032-environment-and-configuration-map-into-options-through-one-core-input-source-stage.md), proposed, supersedes the clause that core evaluates activation from the pre-scan. Activation is evaluated after the input-source stage, from the value each option resolved to, so an option the environment or the configuration source filled activates its middleware exactly as its token would, whatever value the fill holds, and a declared default still activates nothing. The declared list, the absence of a default activation, and deferred loading stand. It also qualifies the Context statement that a value a plugin cannot use is the plugin's own diagnostic: a Boolean plugin option whose variable holds a value outside the closed grammar is a core usage failure, the one input error core raises for a plugin option, because reading that grammar is parsing and not validation. It binds when that record is accepted.
