...
- 2026-09-15: [ADR-0028](0028-plugins-run-code-at-lifecycle-hooks-and-middleware-reads-the-parsed-invocation.md), proposed, supersedes the seam's placement: local parsing and validation run ahead of the chain and their fault is held and raised at `next()`, so a middleware reads the parsed invocation through `input` and a takeover keeps every behavior this record promised, help on an invalid invocation included. The one chain, declared activation, deferred loading, the outcome `next()` resolves, and unwinding order all stand. The rejected option "schemas and presence rules on plugin options" keeps its ruling for options in the globals table; an option a plugin declares on one Command through `onCommandAttach` is a local option and carries a schema like any other.
