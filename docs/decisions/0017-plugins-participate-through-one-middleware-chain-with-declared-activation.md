...
Cleanup is the unwinding side of the chain. A middleware's code after `next()` returns, or in its `finally`, runs in reverse installation order and can read the outcome directly: `next()` returned, `next()` threw, or the run signal is aborted with a reason naming the signal.
