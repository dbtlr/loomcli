...
description: A middleware reads the spelling that supplied each of its own plugin's typed options, and help derives its variant from it, so -h prints a compact page and --help an extended page that adds details, examples, and an ENVIRONMENT block. Core supplies no variant fact. This narrows the ADR-0032 clause that nothing publishes where a value came from.
status: proposed
created: 2026-09-26
modified: 2026-09-26
---
...
- **Help derives the variant.** `-h` selects the compact page, and every other case selects the extended page: `--help`, and an option supplied without a spelling. The last occurrence wins, so `-h --help` is extended and `--help -h` is compact. Core supplies no variant fact and knows nothing of help.
...
- **Extended teaches.** The extended page is the page help prints today, plus an ENVIRONMENT block that lists each visible option on the page bound to a variable. Help prints no configuration binding on either page.
...
- **Configuration bindings on the extended page.** Rejected. Help is not where configuration is taught.
- **A description on the help option that advertises both spellings.** Rejected as not worth the change to every page.

## Consequences

`MiddlewareContext` gains one read-only member and `HelpPage` gains one, so an existing middleware and an existing help replacement compile unchanged. `-h` prints fewer lines, and `--help` prints an ENVIRONMENT block for an application that binds variables. Both are output changes, not breaking changes.
