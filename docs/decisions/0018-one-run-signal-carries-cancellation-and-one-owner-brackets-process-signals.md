...
- 2026-09-25: [ADR-0034](0034-a-declaration-fault-throws-at-the-earliest-point-that-knows-it.md), proposed, supersedes the clause that a second claim on the signals slot fails at build. The Application constructor, which holds every installed plugin, throws it, and `plugin()` rejects a signal outside the closed set or claimed twice. Core still installs its listeners only after the graph builds. It binds when that record is accepted.
