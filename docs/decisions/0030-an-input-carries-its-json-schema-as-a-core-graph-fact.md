...
- 2026-09-25: [ADR-0036](0036-a-collected-inputs-validator-validates-one-item.md), proposed, makes a collected input's validator an item validator. Core then builds that input's schema from the item's published schema, `{ type: 'array', items: <the item's schema> }`, moving the item's `$schema`, and its `$defs` and `definitions` unless it declares `$id`, to the array. That is the one place core writes into the fact; a scalar input's schema stays the converter's return value, unchanged. It binds when that record is accepted.
