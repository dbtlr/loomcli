...
- 2026-09-24: The manifest plugin ships, so `@loomcli/plugins/manifest/extension` no longer ships alone and the superseded complete-plugin clause of ADR-0020 binds again. The manifest reads each Command's collected `manifestCommand` values into its `details` and `examples`, in collection order.
