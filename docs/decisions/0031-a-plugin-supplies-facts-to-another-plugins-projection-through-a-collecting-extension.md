...
- 2026-09-24: Accepted with the implementation. The line and prose rules help and the manifest share live in one pack module outside any subpath, `packages/plugins/src/lines.ts`, which both declarations modules import, so a value help supplies always validates and neither subpath imports the other's internals. `readExtension` keeps one signature whose return type follows the descriptor's `Collect` type. A layer's descriptors register only once all its values are valid, so an `extend()` call a hook catches leaves the build's descriptor registry as it was. The rejection of a hand-built descriptor without a Boolean `collect` is a breaking change with its own fragment.
