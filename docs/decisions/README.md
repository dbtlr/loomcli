...
| [ADR-0023](0023-a-command-declares-its-result-and-core-resolves-its-presentation.md) | A Command declares one value or a sequence of rows with views keyed by presentation name, stdout belongs to the result, and `json` and `jsonl` are views the formatter plugin declares. | proposed |
