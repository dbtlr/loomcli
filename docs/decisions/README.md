...
| [ADR-0031](0031-a-plugin-supplies-facts-to-another-plugins-projection-through-a-collecting-extension.md) | A projection with an open set of suppliers declares a collecting extension whose values accumulate in order, and a supplier adds values from its `onCommandAttach` hook through the declaring plugin's declarations module; help supplies its details and examples to the manifest this way. | proposed |

## Adding a record

ADR-0007, ADR-0008, and ADR-0020 remain accepted except for the specific clauses superseded by [ADR-0027](0027-core-resolves-marked-output-and-one-theme-contribution.md). When proposed [ADR-0031](0031-a-plugin-supplies-facts-to-another-plugins-projection-through-a-collecting-extension.md) is accepted, ADR-0020 and ADR-0025 also remain accepted except for the clauses it supersedes: ADR-0020's clause that a subpath imports nothing from a sibling and, until the manifest plugin ships, its clause that each subpath is a complete plugin; and, for a collecting extension alone, ADR-0025's replacement clause.
