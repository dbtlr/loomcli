...
| [ADR-0023](0023-a-command-declares-its-result-and-core-resolves-its-presentation.md) | A Command declares one value or a sequence of rows with views keyed by view name, stdout belongs to the result, and `json` and `jsonl` are views the format plugin declares. | accepted |
...
| [ADR-0028](0028-plugins-run-code-at-lifecycle-hooks-and-middleware-reads-the-parsed-invocation.md) | Plugins run code at named lifecycle hooks, beginning with `onCommandAttach`, and middleware wraps the parsed invocation and selects a result's view by name. | proposed |
