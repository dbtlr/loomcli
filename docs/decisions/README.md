...
| [ADR-0023](0023-a-command-declares-its-result-and-core-resolves-its-presentation.md) | A Command declares one value or a sequence of rows with named views, stdout belongs to the result, and `json` and `jsonl` are views a formatter plugin declares. | proposed |
