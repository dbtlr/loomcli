...
The wire form delimits a token with private-use code points, one opening the token with its identity and one closing it. Private-use characters do not occur in ordinary data, so nothing is escaped, nesting is a stack that restores the outer token, and stripping is mechanical. This is why interpolated user data cannot break out of or into markup, and why the failure mode that would otherwise need an escaping rule does not exist. The `width` and `pad` helpers exist because the native string methods count markup, which is the one trap a table cell walks into.
...
Escaping user data is prevented by the wire form rather than mitigated by a rule. Color-only meaning is prevented by core owning the glyphs.
