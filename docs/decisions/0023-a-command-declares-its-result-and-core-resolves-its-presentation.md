---
type: adr
title: ADR-0023 - A Command declares its result as named views, stdout belongs to the result, and formatters are views a plugin declares
...
# ADR-0023 - A Command declares its result as named views, stdout belongs to the result, and formatters are views a plugin declares
...
`json` and `jsonl` are views. The formatter plugin declares them as configured factories with a per-row map, or a whole-value map under `result`, that reshapes the wire form the way a column list reshapes a table. They render as ordinary views with the plugin uninstalled. Installing the plugin adds `--format` selection over the record's keys and makes both available on every declared result with the identity map. Core knows no presentation name, not even `text`, and there is no encoding outside the view model, so an agent asking for a presentation by name reaches a view whichever package declared it.
...
A declared result is a promise the Command makes. An action that returns normally without calling `out.results` fails with `ResultError`, exit 1, naming the Command. A failure raised before the call is that failure, because a failure is the outcome. A second call is a `ResultError` that turns a would-be 0 into 1, the rule a second `next()` follows in the middleware chain. A rows source that fails partway leaves written rows in place under a row view and an empty stdout under a whole view, and core writes one line through `@loomcli/core/results/incomplete` before the failure's diagnostic, so an empty stdout and a failed stdout never read the same.
...
- **A per-Command list of the formatters it supports.** Rejected. The record of views is that list.
...
The formatter plugin's contract, its subpath, the `--format` spelling, the `ndjson` alias of `jsonl`, the usage error on a Command with no result, and how help lists the available presentations, is a separate phase, as is the configuration each pack view accepts and whether the table is a row view with declared widths or a whole view.
