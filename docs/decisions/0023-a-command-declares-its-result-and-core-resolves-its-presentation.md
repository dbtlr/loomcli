...
Core resolves the run's presentation. It uses the declared default view unless an installed formatter claimed the run, or unless a `--format` selection named another declared view. Core knows no format name, not even `text`. Plugins own format names and the encoders behind them, which is the split ADR-0018 uses for the signals slot and the rule ADR-0013 states for plugins in general: core declares the lane, and nothing first-party is privileged inside it.
