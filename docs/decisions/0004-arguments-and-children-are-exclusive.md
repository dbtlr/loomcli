...
- 2026-09-15: [ADR-0028](0028-plugins-run-code-at-lifecycle-hooks-and-middleware-reads-the-parsed-invocation.md), proposed, moves the check again: it is judged before the chain, held, and raised at the dispatch boundary, so a plugin still takes over a group invocation before the error is raised and the class, code, and rank are unchanged. It binds when that record is accepted.
