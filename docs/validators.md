...
**Defaults.** A default is a raw string in the validator's input type, `default: '8080'` for `port()`, and passes through the validator like a token.

**Published input schemas.** A factory publishes the JSON Schema of the value its token stands for, under the rule of [Input schema](core.md#input-schema): `integer()` publishes `type: 'integer'`, and `url()` publishes the string. `'~standard'.jsonSchema.input({ target: 'draft-2020-12' })` returns a new plain object with `$schema: 'https://json-schema.org/draft/2020-12/schema'` beside the keywords each factory lists below. Any other target throws, as the standard permits. `'~standard'.jsonSchema.output` throws for every target, because the output side is the action's business and core never asks for it.
...
**Faults at the call.** A factory argument that can never work throws a `DeclarationError` from the factory call, under [ADR-0034](decisions/0034-a-declaration-fault-throws-at-the-earliest-point-that-knows-it.md). Each factory lists its faults. TypeScript rejects what its types can express; the runtime check covers the rest for JavaScript callers and widened values.
...
- **Messages.** Without `pattern`, the message states the length rule: `Expected a nonempty value.`, `Expected at most 32 characters.`, `Expected from 2 through 32 characters.`, or `Expected at least 2 characters.` for a `minLength` above 1. With `pattern`, a length failure keeps its own sentence and a pattern failure reads `message`, or `Expected a value that matches the required pattern.` when none is given.
...
A token that begins with `-` is read by the parser as an option unless it is attached: `--offset=-5` supplies `-5`, and `--offset -5` does not.
...
- **Publishes** `{ type: 'string', format: 'uri' }`. With `protocols`, it adds a `pattern` anchored at the start that lists each scheme followed by `:`, spelling each letter as a two-case class, so `['https']` publishes `^(?:[hH][tT][tT][pP][sS]):`.
- **Message.** `Expected an absolute URL, such as https://example.com.`, or, with `protocols`, `Expected an absolute URL with the scheme https.` and `Expected an absolute URL with the scheme https or http.` for two or more.
...
- **Resolves** a nonempty token against `host.cwd` under the path rules of `host.platform`, and normalizes it. It does not resolve symbolic links. The empty string is rejected.
...
- **Verdicts and faults.** A probe that reports the entry missing, not a directory along the way, not permitted, or a symbolic-link loop is a verdict, and the token is rejected. A probe that fails any other way throws, and core reports it as a validator failure under [Issues and validator failures](core.md#issues-and-validator-failures).
...
- `@loomcli/core` is a peer dependency, as it is for `@loomcli/plugins`. The package imports the Standard Schema types, `ValidationContext`, and `validationContext` from core, and adds no runtime dependency of its own.
- The package joins the synchronized release set the way `@loomcli/plugins` did under ADR-0037: it lands with `private: true`, the maintainer publishes a `0.0.0` placeholder and binds the npm trusted publisher, an ordinary pull request removes `private` at the current synchronized version, and the next release cut publishes it. The root `build` script and `scripts/clean.mjs` list it.
...
- textstat's `files` rule, a nonempty list or piped stdin, moves into its action under ADR-0036: an empty list with a terminal on stdin throws `InputError` with the message `Supply file arguments or pipe text to stdin.` and exit code 2.
...
- **Soundness.** For each factory, a table of accepted tokens validates, under a JSON Schema draft 2020-12 validator with format assertion on, the value the token stands for against the published schema: the number for `integer`, `number`, and `port`, and the token for every other factory. A test fails when any accepted token breaks the schema. The table includes each factory's edges: code points outside the Basic Multilingual Plane for `text`, `-0`, leading zeros, and safe-integer limits for the numbers, the leap day for `date`, mixed case for `uuid` and a `protocols` scheme, and for `url` the tokens RFC 3986 rejects and the WHATWG parser repairs.
...
- **Help.** A `oneOf` input prints `One of: ...` with no authored `accepts`.
