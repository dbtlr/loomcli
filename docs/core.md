...
| `ResultError`             | `InternalError` | 1 | `path`, `kind`; see [Results](#results) |
...
views(replacements: ResultViews<Value> | RowViews<Row>, options?: { default?: string }): …

// The result rides in the declared types beside args, options, and globals.
...
A rows sequence, under `out.results` or under `out.render` with a row view, can stop before its source ends: the source throws, a row view's function throws or returns a non-string, a write to the destination fails, the action fails while an unawaited sequence is pending, or the run is cancelled and the source then ends. In every case core stops requesting rows, writes no `tail`, and retracts nothing already written, so under a row view the rows already written stay on stdout and under a whole view stdout is empty. Core then writes one line on stderr through the declared view `incompleteResult`, exported from core over `{ path, rows }`, where `rows` counts the rows the source yielded before the sequence stopped, `head` and `tail` excluded, under a whole view as under a row view. It is written the way a failure diagnostic is: the view owns its newline, and an override that returns the empty string silences it. The line prints with `0 rows` too, because an empty stdout and a failed stdout must not read the same.

```text
Output is incomplete: Command "count" stopped after 3 rows.
```

What follows the line is the fault's own report. A source that throws ends the invocation with that failure's exit code and its diagnostic. A row view that throws or returns a non-string is the view failure of the [Failure contract](#failure-contract), an internal error with exit 1, and the atomic rule there, nothing written for the call, is scoped to a whole view; a row view's pieces before the fault stand. A write failure follows the destination row of the same contract. An action that fails while its unawaited sequence is pending stays primary, and the sequence is stopped rather than drained. Under cancellation the code stays the signal's, a cancellation echo the source throws is silent, and a source that ends after the signal aborted, by returning or by throwing, is treated as stopped early, so a truncated result never reads as a complete one. A failure the action raises after `out.results` resolved is an ordinary failure, and the result it already wrote stands.
...
- `out` follows the output contract an action has, with the default destinations on every Command and no `results` method, since a result is the action's promise under [Results](#results). Output a middleware issues counts toward completion the same way, and a middleware whose promise never settles holds the run open exactly as an action would.
