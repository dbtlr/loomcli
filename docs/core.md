...
- A converter fails when it throws or returns anything but a plain object. `inspect()` reports the failure as a `DeclarationError` naming the input, the target, and the converter's message: `Command "sizes" option "minimum" declares a schema whose converter failed for target "draft-2020-12": Transforms cannot be represented in JSON Schema. Declare a schema the converter describes.` The subject follows the forms the [Graph build errors](#graph-build-errors) table uses: a global option reads `Global option "file" declares a schema ...`, an argument reads `Argument "files" on Command "count" declares a schema ...`, and the root reads `The root Command` in place of a named one. A non-object return reads `returned a value that is not a plain object` in place of the message. `run()` applies no such rule: it stores `null` and continues, because an operator cannot correct an author's schema, and a projection reads the unknown shape. This is the one rule `inspect()` applies that `run()` does not, the mirror of the declared default that `run()` alone checks.
