...
The renderer receives the failure instance and returns the diagnostic core writes to stderr. Like `out.render`, the renderer owns every byte core writes, the trailing newline included: core appends nothing and strips nothing. A renderer cannot change the exit code, which is a fact of the class.
...
Successful completion requires output completion. A renderer failure during the action makes the invocation unsuccessful even when the action returned normally and even when it caught the rejection. The fallback path never runs application code.
