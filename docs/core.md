...
| A hidden or deprecated fact on the root or an argument  | `The Application declares hidden, which applies to named Commands and options alone. Remove it.` The same sentence names the fact declared, and an argument as `The root Command argument "files"` or `Command "get" argument "path"`. The Application options reject both keys at compile time; a TypeScript author reaches this rule through an argument config held in a variable, because excess-key checking applies to a fresh object literal alone. |
