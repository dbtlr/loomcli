...
```ts
// Both option configs, `StringOption` and `BooleanOption`, accept `env` on every kind of option.
// A multiple string option does not, so `{ multiple: true, env: 'FIELDS' }` fails to compile.
