...
| A description that is blank or holds a line terminator  | `Command "get" description must hold a character other than whitespace and no line terminator. Supply a one-line summary.` The same sentence names the Application, an option, or an argument. |
...
Renderer and destination failures are internal errors and return code 1, except in a cancelled run that reached the middleware chain, where the code stays the signal's and the fault is reported as text; every row below reads with that one carve-out.
