...
| An override key from a second copy of a package         | `The Application overrides view "@loomcli/plugins/help/page" through a declaration that is not the one on the graph. Install one copy of the package that declares it.`                                                                                                  |
...
type AnyDeclaredView = View<never> & DeclaredViewBrand & { readonly identity: string }; // the private brand without the invariance witness
...
Build applies three rules to the registry, each a `DeclarationError` at build, reported through the application's override for `DeclarationError` when the fault lies in a plugin's `views` and through core's default text when it lies in the application's own, as [Failure views](#failure-views) explains: two overrides for one key inside one contributor, two distinct declared-view objects that share one identity, and a `views` entry that is not an override on the Application, or neither a declared view nor an override on a plugin, which the types already reject and a JavaScript author alone reaches. [Graph build errors](#graph-build-errors) and [Plugin build errors](#plugin-build-errors) list the diagnostics. The registry is not an `inspect()` fact in this contract.
