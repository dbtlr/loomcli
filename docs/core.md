...
JavaScript authors reach the same rules at graph build, which reports a declaration made after the action, and arguments declared beside children. Both are listed in [Graph build errors](#graph-build-errors). A plugin's [Commands](#plugin-commands) attach outside the authoring types, so a root that declares arguments reaches the arguments-beside-children rule at build for every author when a plugin brings Commands.
...
- **One source.** An application has at most one configuration source. A second installed plugin that declares one is a build error naming both plugins, as a second claim on the [signals slot](#signals-and-cancellation) is.
...
const doctorCommand = new Command('doctor', { description: 'Check the host this application runs on.' }).action(
  ({ out }) => out.info('All checks passed.'),
);
...
- **Where and when.** At graph build, core attaches each installed plugin's Commands to the root, in plugin installation order and in list order within one plugin, ahead of the application's own Commands. The root's authoring order is therefore the plugins' Commands first, and help's command rows, `inspect()`, and the manifest list them in that order.
- **An ordinary Command.** Every Command rule applies to a plugin Command unchanged, at every depth of its subtree: naming, [aliases](#aliases), hidden and deprecated, routing, parsing, the [input-source stage](#input-sources), validation, and every row under [Graph build errors](#graph-build-errors). Every installed plugin's [lifecycle hooks](#lifecycle-hooks) run on it, its own plugin's included, in the root-first, depth-first walk. Nothing in the graph records which plugin attached it. Its action receives what every action receives, which excludes the plugin's own options, and runs only when routing selects its Command.
...
- **Collisions.** A plugin Command whose name or alias repeats another root child's, from the application or from another plugin, fails the build with the sibling rows under [Graph build errors](#graph-build-errors), such as `The root Command attaches two children named "doctor". Rename or remove one.` An application that also attaches the plugin's Command value to the root fails with the two-children row, and one that attaches it under another Command fails with the one-parent row.
- **A root with arguments.** A root that declares arguments cannot also hold children, so an application whose root declares arguments cannot install a plugin that brings Commands. The build fails with the existing row, `The root Command declares argument "files" and attaches child "doctor". Move the argument into a child Command or remove the children.`
...
The acceptance attaches a fixture plugin's Command to jsonkit's root. The fixture plugin compiles in its own package, outside jsonkit's `Register` augmentation, as the private `@loom/explain` plugin does. `jsonkit doctor` routes and runs it, `jsonkit --help` lists it ahead of jsonkit's own Commands, `inspect()` and `--manifest` list it as the root's first child, `jsonkit doctor --help` prints its page, and a hook-declared option reaches it. Build fails when the application attaches a Command with the same name, when two installed plugins attach one name, when an alias repeats a sibling's name, when the application also attaches the plugin's Command value under another Command, and when the root declares arguments. Each case runs under Node and Bun.
