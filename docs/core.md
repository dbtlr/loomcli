...
export function format({ option = 'format' }: { option?: string } = {}): Plugin {
  return plugin(`${Package.name}/format`, {
    middleware: {
      activate: 'always',
      load: async () => ({ default: (await import('./middleware.js')).selectFormat(option) }),
    },
    onCommandAttach: attachFormat(option),
  });
}
```

The configured spelling reaches both halves of the plugin from the entry module: the hook closes over it, and `load` hands it to the middleware module, which exports a factory rather than a fixed default, since a lazily loaded module has no other way to learn what the entry was configured with.
...
export function selectFormat(option: string): Middleware<typeof format> {
  return async (context) => {
    const selected = context.input?.options[option];
