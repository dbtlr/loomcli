...
- **Derived.** Without `accepts`, help derives the sentence from the input's [input schema](#input-schema) when that schema is a closed set of strings: an `enum` whose members are all strings, a single string `const`, or an `anyOf` whose members are each such an `enum` or `const`, flattened in order, and for a multiple option or a variadic argument the same shapes under `items`, because each token must be one of the values. Help reads `enum`, `const`, and `anyOf` at the schema's top level, or under `items` for a collection, ignores every other keyword, such as `type`, `default`, `description`, or `minItems`, and derives nothing when more than one of the three sits at the same level. The sentence is `One of: ` followed by the values in the schema's order, separated by a comma and a space, and a closing period: `One of: bytes, words, lines.`
...
- **Quoting.** A value that is empty or holds whitespace, a comma, or a double quote prints as its JSON string, `One of: "a b", c.`, so the list splits unambiguously, and a line terminator inside it prints as its JSON escape, as a rendered default's does, so a row stays one line. Every value is escaped before styling, as every graph string on the page is.
...
- **Derived shapes.** Fixture applications cover a string `enum`, a single string `const`, an `anyOf` of string `const`s, an `anyOf` mixing an `enum` and a `const`, an `enum` beside a `default` keyword, a repeated value printing once, an empty set and a nullable enum deriving nothing, the same set under `items` for a multiple option and a variadic argument, eight values printing, nine deriving nothing, a set holding a number deriving nothing, a pattern deriving nothing, and an argument row deriving from its schema.
...
- **Text.** A value that is empty or holds a space, a comma, a double quote, or a line terminator prints as its JSON string, a marker character in a value prints literally, and a row with no description starts its right cell with the sentence.
