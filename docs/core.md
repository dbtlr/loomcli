...
```json
{
  ...
  "version": "0.0.0",
  "description": "Read and reshape one JSON document.",
  "tokens": "Every input is a string token. A schema describes the value one token must satisfy, or the whole list of tokens for a multiple option or a variadic argument.",
  "exitCodes": {
    "0": "Successful execution and core output",
    "1": "Expected action failure, internal failure, or invalid declarations",
    "2": "Invalid invocation inputs",
  ...
  "encodings": {
    "json": "A result of kind value prints as one JSON document, and a result of kind rows prints as one JSON array of its rows.",
    "jsonl": "A result of kind rows prints one JSON document per line, one line per row and nothing for no rows, and a result of kind value prints as one line."
  },
  "globals": [
    {
      "type": "string",
...
- **The takeover.** The middleware prints the document for the routed Command and returns without calling `next()`, so the exit code is 0, the action never dispatches, and a fault core held from parsing or validation is never raised: `jsonkit cache --manifest` prints the group's document, and `jsonkit get --manifest` prints while `path` is missing. An unknown Command still fails in routing, so `jsonkit nope --manifest` reports the unknown command, and a pre-scan structure fault still ranks ahead of the chain. When help is also installed, installation order decides which takes over, so `jsonkit --help --manifest` prints help in the example applications.
...
- **The envelope.** `name`, `version`, and `description` are the graph's. `tokens` states the token rule once, so no entry repeats it. `exitCodes` carries the five codes with the meanings the [Invocation](#invocation) table gives them, and names no failure class. `encodings` states what the `json` and `jsonl` view names promise under [Declaring a result](#declaring-a-result). The three statements are fixed strings, the same in every document.
...
- **An input entry.** An argument entry mirrors `ArgumentNode` without `validated`, `validateOmitted`, and `extensions`. An option entry mirrors its `OptionNode` variant without `hidden`, `scope`, `validated`, `validateOmitted`, and `extensions`, so a plugin option and an application option read alike and the document names no plugin. `schema` is the graph's [input schema](#input-schema), copied verbatim. `default` is `{ "value": <declared value> }`, or `null` when the input declares no default or declares `undefined`. A hook-declared option, `--format` included, is an ordinary local option entry.
...
- **Key order.** Every object the plugin builds holds its keys in the order the type block lists them. `result` holds `kind`, `views`, and `default` in that order. A `schema` object keeps the order its converter produced.
- **Bytes.** The document is `JSON.stringify(document, null, 2)` and one newline, written to stdout with no style, so the bytes are the same under every capability. The text is escaped as the formatter's `json()` escapes it: through `style.escape`, with every character from U+007F to U+009F replaced by its `\uXXXX` escape in four lowercase hex digits. A declared default JSON cannot encode, such as a `bigint`, fails the write through the output-view row of the [Failure contract](#failure-contract). The middleware writes through a bare view the plugin does not declare, so no override reaches it: the document is data, as a result's `json` output is, and an application that wants another document writes its own projection.
...
- **Takeover and precedence.** `--manifest` on a group, and on a Command missing a required argument, prints with exit 0. An unknown Command still reports its routing error. `--help --manifest` prints help. A `--manifest` token after `--` is not read as the option.
- **Entries.** A hidden option and a hidden global are omitted. An explicit `default: undefined` reads `null`. Two `manifestCommand` values with `details` produce two strings in collection order, and a value with neither field adds nothing. A U+009B inside a description prints as `\u009b`. A declared `bigint` default fails through the output-view row. Keys follow the type block's order, `result` included, and no version field appears.
