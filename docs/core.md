...
- **Values.** A string option takes a JSON string as it is, or a JSON number as `JSON.stringify` writes it, so `5` fills `"5"` and the option's validator decides; a literal such as `1e400` that overflows to infinity is no JSON number and is a wrong value. A Boolean option takes a JSON `true` or `false`, which states the option's value, not a spelling, under every polarity, as an environment value does. A multiple option takes an array whose items each follow the string rule, and an empty array is a fill. Any other value, `null`, an object, a Boolean for a string option, a string for a Boolean option, or a bare value for a multiple option, is a wrong value.
