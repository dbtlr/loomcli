...
- **Derived shapes.** Fixture applications cover a string `enum`, a single string `const`, an `anyOf` of string `const`s, an `anyOf` mixing an `enum` and a `const`, an `enum` beside a `default` keyword, an `enum` beside a `pattern` or `minLength` deriving nothing, a repeated value printing once, an empty set and a nullable enum deriving nothing, the same set under `items` for a multiple option and a variadic argument, an array-level `prefixItems` or `contains` deriving nothing and an array-level `default` or `description` still deriving, eight values printing, nine deriving nothing, a set holding a number deriving nothing, a pattern deriving nothing, and an argument row deriving from its schema.
