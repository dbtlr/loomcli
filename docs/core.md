...
JavaScript authors, and any author whose root declares arguments and installs a plugin that brings [Commands](#plugin-commands), reach the same rules at graph build, which reports a declaration made after the action, and arguments declared beside children. Both are listed in [Graph build errors](#graph-build-errors).
