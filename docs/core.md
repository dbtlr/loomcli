...
```ts
interface AttachedCommand extends AttachedCommandBrand {
  readonly name: string | null;
  readonly path: readonly string[];
  readonly hasAction: boolean;
...
The configured spelling reaches both halves of the plugin from the entry module: the hook closes over it, and `load` hands it to the middleware module, which exports a factory rather than a fixed default, since a lazily loaded module has no other way to learn what the entry was configured with. The factory is annotated `Plugin<{}>`, not bare `Plugin`, because `Plugin`'s default is the open options record, under which `context.options.anything` compiles and is always `undefined`; `Plugin<{}>` rejects it, and every plugin that declares no options annotates its factory that way.
...
An empty call returns an equivalent new declaration. Extending preserves the action, inputs, aliases, children, core facts, Application environment, and authoring state. It never reopens input or action declarations. Core facts such as `description`, `hidden`, and `deprecated`, and option/argument extensions, retain their constructor or input-configuration rules. A plugin reshapes a completed declaration through `onCommandAttach` under [Lifecycle hooks](#lifecycle-hooks), not through `extend()`.
